...
/// <summary>
/// A cache implementation with time-based expiration support.
/// Entries are automatically removed after a configured duration since write or access.
/// </summary>
internal sealed class ExpiringCache<K, V> : ICache<K, V> where K : notnull
{
    ...
    public ExpiringCache(Caffeine<K, V> builder)
    {
    ...
    public V? Get(K key, Func<K, V?> mappingFunction)
    {
        ...
        var newEntry = new TimedEntry<V>(newValue, now);
        _map[key] = newEntry;
        return newValue;
    }
    ...
    public IReadOnlyDictionary<K, V> GetAll(IEnumerable<K> keys, Func<ISet<K>, IDictionary<K, V>> mappingFunction)
    {
        ...
        if (missingKeys.Count > 0)
        {
            ...
            {
                var timedEntry = new TimedEntry<V>(entry.Value, now);
                _map[entry.Key] = timedEntry;
